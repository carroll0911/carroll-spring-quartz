...
public class TaskServiceImpl implements ITaskService {
    ...
    public Boolean addTask(TaskEntity info) throws QuartzException {
     ...
     */
    @Override
    public Boolean resumeTask(TaskEntity taskEntity) throws QuartzException {
        try {
            scheduler.resumeJob(JobKey.jobKey(taskEntity.getJobName(), taskEntity.getJobGroup()));
            return true;
        } catch (Exception e) {
            throw new QuartzException("开始定时任务失败");
        }
    }
     ...
     * @param info
     */
    public Boolean updateTask(TaskEntity info) throws QuartzException {
        String jobName = info.getJobName(),
                jobGroup = info.getJobGroup(),
                cronExpression = info.getCronExpression(),
                jobDescription = info.getJobDescription(),
                createTime = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));//DateFormatUtils.format(new Date(), "yyyy-MM-dd HH:mm:ss");
        try {
            if (!checkExists(jobName, jobGroup)) {
     ...
     */
    @Override
    public Boolean pauseTask(TaskEntity taskEntity) throws QuartzException {
     ...
     * @// TODO: 2018/6/7 删除任务
     */
    public Boolean deleteTask(TaskEntity taskEntity) throws QuartzException {
        TriggerKey triggerKey = TriggerKey.triggerKey(taskEntity.getJobName(), taskEntity.getJobGroup());
        try {
            if (checkExists(taskEntity.getJobName(), taskEntity.getJobGroup())) {
                scheduler.pauseTrigger(triggerKey); //停止触发器
